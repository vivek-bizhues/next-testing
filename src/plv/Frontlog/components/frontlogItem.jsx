...
} from "../../../store/plv2/backlogFrontlog/frontlog";
import { Dialog, DialogActions, DialogTitle } from "@mui/material";
import { ToastContainer } from "react-toastify";

const FronglogItem = (props) => {
  ...
  const getRows = () => {
    ...
    const frontlog_factoring = [];
    backlog_project_name.push(
      headerCell("Project", "justify-content-left color-grey "),
      textCell(props.project.name, "justify-content-end  ")
    );
    backlog_project_start_date.push(
      headerCell("Start Date", "justify-content-left color-grey"),
      dateCell(
    ...
    backlog_project_end_date.push(
      headerCell("End Date", "justify-content-left color-grey"),
      dateCell(
        new Date(convertToUTCDate(props.project.end_date)),
        "justify-content-end"
      )
    );
    frontlog_factoring.push(
      headerCell("Factoring %", "justify-content-left color-grey"),
      percentCell(props.project.flfactoring / 100, "justify-content-end")
    );

    // 2 columns for attribute and attribute value
    header_cells.push(
      headerCell("", "justify-content-left color-grey"),
      headerCell("", "color-grey")
    );

    Array.isArray(backLogFormattedDateRange) &&
      backLogFormattedDateRange.map((d) => {
        header_cells.push(
          nonEditable(headerCell(d, "justify-content-center color-grey", 2)),
    ...
    contractValue_cells.push(
      ...
      numberCell(props.project.contract_val, "justify-content-end ")
    );
    ...
    props.project &&
      ...
      Array.isArray(props.project.transactions) &&
      props.project.transactions.map((tr) => {
        ...
        contractValue_cells.push(
          nonEditable(numberCell(tr.month_val, "justify-content-end ")),
          nonEditable(rightBorder(emptyTextCell))
        );
        ...
        materials_cells.push(
          nonEditable(
            numberCell(
              tr.material_val,
              getValueCssClass(tr.material_val, "justify-content-end")
            )
          ),
          rightBorder(
    ...
    materials.push(
      ...
      percentCell(props.project.material_c_per / 100, "justify-content-end")
    );

    subContracts.push(
      nonEditable(textCell("Sub Contracts %", "justify-content-left disabled")),
      percentCell(props.project.subcontract_c_per / 100, "justify-content-end")
    );

    directLabour.push(
      nonEditable(textCell("Direct Labour %", "justify-content-left disabled")),
      percentCell(props.project.directlabour_c_per / 100, "justify-content-end")
    );
    ...
    const rows = [
      ...
      {
        rowId: "flfactoring__" + props.project.id,
        height: ROW_HEIGHT,
        cells: frontlog_factoring,
      },
      ...
      if (change.columnId === "attribute_value") {
        ...
        switch (attribute) {
          ...
          case "contract_val":
            ...
            break;
          case "flfactoring":
            attribute_name = "flfactoring";
            if (change.newCell.value > 100 || change.newCell.value < 0) {
              toast.error("Factoring can be between 0% and 100%");
              return false;
            }
            break;
  ...
  const handleCloneFrontlogProject = () => {
    dispatch(
      cloneFrontlog({
        id: props.project.id,
        filter: props.filter,
    ...
    // toast.success(`Project ${props.project.name} is cloned`);
  };
          ...
          <>
            ...
            <div className="flex gap-4">
              ...
              <Button
                ...
                size="small"
                color="secondary"
                onClick={handleCloneFrontlogProject}
              >
                Clone
              </Button>
              ...
              >
                <DialogTitle id="alert-dialog-title">
                  Are you sure to move this project to Backlog?
                </DialogTitle>
