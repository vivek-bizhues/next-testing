...
import {
  ...
  updateFixedAssetStraightLinePoolTransactionData,
  updateFixedAssetStraightLinePoolLedger,
} from "../../../../store/plv2/fixedAssetStraightLinePoolTransactions";
...
export default function FixedAssetStraightLinePoolsLedgerView(props) {
  const ROW_HEIGHT = 32;
  ...
  const fixedAssetStraightLinePoolTransactionStore = useSelector(
    (state) => state.fixedAssetStraightLinePoolTransactions
  );
  ...
  }, [props.show, dateRangeInMonths, props.poolId]);

  useEffect(() => {}, [fixedAssetStraightLinePoolTransactionStore]);

  useEffect(() => {
    prepareDF();
  }, [columns, fixedAssetStraightLinePoolTransactionStore]);
  useEffect(() => {
    prepareRows();
  }, [dataFrame]);
  ...
  const prepareDGColumns = () => {
    ...
    _headerCells.push(
      nonEditable(
        rightBorder(headerCell("Additions", "justify-content-end wraptext"))
      )
    );

    _headerMonthCells.push(nonEditable(headerCell("", "")));
    _headerMonthCells.push(nonEditable(rightBorder(headerCell("", ""))));
  ...
  const prepareDF = () => {
    ...
    if (rowsList.length > 0 && columnIds.length > 0) {
      ...
      if (df.columns.length > 0 && df.values.length > 0) {
        fixedAssetStraightLinePoolTransactionStore.data.forEach((asset) => {
        ...
        <DialogTitle>
          <Grid container spacing={2}>
            {isLoading ? (
              <>
                <Grid item xs={7}>
                  Straight Line Fixed Asset Pool [{props.title}]
                </Grid>
                <ReactLoaderRound />
              </>
            ) : (
