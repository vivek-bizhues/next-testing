...
import { fetchFixedAssetDecliningBalancePoolTransactionTotalData } from "../../../../store/plv2/fixedAssetDecliningBalancePoolTransactionsTotal";
...
export default function FixedAssetDecliningBalancePoolsTotalLedgerView(props) {
  const ROW_HEIGHT = 32;
  ...
  const fixedAssetDecliningBalancePoolTransactionsTotaltore = useSelector(
    (state) => state.fixedAssetDecliningBalancePoolTransactionsTotal
  );
  ...
  useEffect(() => {
    if (props.show === true) {
      setIsLoading(true);
      dispatch(
        fetchFixedAssetDecliningBalancePoolTransactionTotalData({
  ...
  useEffect(() => {
    prepareDF();
  }, [columns, fixedAssetDecliningBalancePoolTransactionsTotaltore]);
  useEffect(() => {
    prepareRows();
  }, [dataFrame]);
  ...
  const prepareDF = () => {
    ...
    const rowIndexes =
      fixedAssetDecliningBalancePoolTransactionsTotaltore.data.map((pool) => {
    ...
    if (rowsList.length > 0 && columnIds.length > 0) {
      ...
      if (
        df.columns.length > 0 &&
        df.values.length > 0 &&
        fixedAssetDecliningBalancePoolTransactionsTotaltore.data.length
      ) {
        fixedAssetDecliningBalancePoolTransactionsTotaltore.data.forEach(
          (pool) => {
      ...
      <DialogTitle>
        <Grid container spacing={2}>
          {isLoading ? (
            <>
              <Grid item xs={7}>
                Total All Declining Balance Fixed Asset Pool
              </Grid>
              <ReactLoaderRound />
            </>
          ) : (
